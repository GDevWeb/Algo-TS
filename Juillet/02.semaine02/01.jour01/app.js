...
function reverseString(paramString) {
    try {
        ...
        // main logic :
        // clean string :
        const reversedString = Array.from(paramString)
            .reverse()
            .toString()
            .replaceAll(",", "");
 ...
 */
function isPairOrUnPair(paramNumberArray) {
    try {
        if (paramNumberArray === undefined ||
        ...
        if (paramNumberArray.some((element) => !Number.isInteger(element))) {
            throw new Error(`Veuillez saisir un tableau contenant uniquement des nombres entiers`);
        }
        if (paramNumberArray.some((element) => element < 0)) {
            throw new Error(`Veuillez saisir un tableau contenant uniquement des nombres entiers positif et supérieur à 0`);
        }
        // main logic :
        let resultArray = [];
